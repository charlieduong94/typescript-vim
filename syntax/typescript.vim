...
"" typescript class"{{{
"syntax match typescriptClassName /\<\K\k*\ze\s*{/
syntax match typescriptClassName /\<\K\k*\>/ contained
"}}}
"" typescript variable type"{{{
syntax match typescriptVariableType /\<\K\k*\>/ contained
syntax match typescriptVariableTypeSeparator /:/ contained nextgroup=typescriptVariableType skipwhite
syntax match typescriptVariable /\<\K\k*\>/ contained nextgroup=typescriptVariableTypeSeparator skipwhite
"}}}
"" typescript Function call"{{{
syntax match typescriptFunctionCall /\<\K\k*\ze\s*(/
"}}}
...
syntax keyword typescriptVariableDeclaration let var const nextgroup=typescriptVariable skipwhite
...
syntax keyword typescriptReserved constructor declare as interface module abstract enum int short export interface static byte extends long super char final native synchronized float package throws goto private transient debugger implements protected volatile double import public type namespace from get set keyof

syntax keyword typescriptReservedKeywordClass class nextgroup=typescriptClassName skipwhite
"}}}
...
if version >= 508 || !exists("did_typescript_syn_inits")
  ...
  HiLink typescriptVariableDeclaration Identifier
  ...
  HiLink typescriptClassName Special
  HiLink typescriptFunctionCall Function
  HiLink typescriptVariableType Type
  HiLink typescriptReservedKeywordClass Keyword
